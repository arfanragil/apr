...
import { Plus, Search, Filter, Trash2, X, Download, FileSpreadsheet, Paperclip } from 'lucide-react'
...
export default function PengeluaranClient({ initialData, categories }: { initialData: Expense[], categories: Category[] }) {
  ...
  const filteredData = initialData.filter(item => {
    ...
    const matchesCategory = filterCategory ? item.expense_categories?.name === filterCategory : true
  ...
  const handleExport = () => {
    const exportData = filteredData.map(item => ({
      ...
      'Tanggal': item.date,
      'Kategori': item.expense_categories?.name || '-',
      'Keterangan': item.description || '-',
      'Nominal (Rp)': item.amount,
      'Dibuat Oleh': item.users?.full_name || '-',
      'URL Bukti': item.proof_url || '-'
    }))

    const ws = XLSX.utils.json_to_sheet(exportData)
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, "Pengeluaran")
    XLSX.writeFile(wb, `Laporan_Pengeluaran_${new Date().getTime()}.xlsx`)
  }
  ...
  const handleAdd = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (res.success) {
      ...
      alert(res.error || 'Gagal menambahkan pengeluaran')
    }
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Pengeluaran Kas</h1>
          <p className="text-gray-500 text-sm">Pencatatan uang keluar operasional perumahan.</p>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <button 
            ...
            className="flex-1 sm:flex-none bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-red-700 flex items-center justify-center gap-2 shadow-sm"
          >
            <Plus size={16} /> Tambah
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500 font-medium">Total Pengeluaran (Sesuai Filter)</p>
              ...
              {filteredData.length === 0 ? (
                ...
                filteredData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 bg-red-50 text-red-700 rounded-full text-[10px] font-bold uppercase">
                    ...
                    <td className="px-6 py-4 text-center">
                      {item.proof_url ? (
                        <a href={item.proof_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline flex items-center justify-center gap-1">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <h3 className="font-bold text-lg text-red-600">Catat Pengeluaran</h3>
            ...
            <form onSubmit={handleAdd} className="p-4 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input name="amount" type="number" required placeholder="Contoh: 150000" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 outline-none font-semibold text-lg" />
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-sm font-medium text-gray-700">Tanggal *</label>
                  <input name="date" type="date" required defaultValue={new Date().toISOString().split('T')[0]} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-sm font-medium text-gray-700">Kategori *</label>
                  <select name="category_id" required className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 outline-none bg-white">
              ...
              <div className="space-y-1">
                ...
                <textarea name="description" rows={3} placeholder="Catatan tambahan (opsional)" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 outline-none"></textarea>
              </div>

              <div className="space-y-1">
                <label className="text-sm font-medium text-gray-700">Upload Nota (Opsional)</label>
                <input 
                  type="file" 
                  accept="image/*,.pdf"
              ...
              <div className="pt-4 mt-6 border-t border-gray-100 flex justify-end gap-2">
                ...
                <button type="submit" disabled={isSubmitting} className="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 disabled:opacity-70">
